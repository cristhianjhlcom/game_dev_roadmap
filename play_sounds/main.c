...
void cleanup()
{
    ...
    Mix_FreeChunk(state.pop_sound);
    TTF_CloseFont(state.font);
    state.font = NULL;
    state.renderer = NULL;
    state.window = NULL;
    state.player.texture = NULL;
    state.pop_sound = NULL;
    TTF_Quit();
    Mix_Quit();
    SDL_Quit();
}
...
bool load_media(void)
{
    ...
    state.pop_sound = Mix_LoadWAV("assets/sounds/pop.wav");
    if (state.pop_sound == NULL)
    {
        SDL_Log("Load game pop sound failed %s\n", Mix_GetError());
        return false;
    }
...
void mouse_handler(SDL_Event *event)
{
    ...
    state.player.w = 100;
    state.player.h = 100;
    Mix_PlayChannel(-1, state.pop_sound, 0);
    state.click_score++;
}
